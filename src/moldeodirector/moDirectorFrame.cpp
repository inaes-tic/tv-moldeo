...

{
  ...
  wxDir SampleDir("../../data/samples");
  wxFileName dirname( wxT("../../data/samples"), wxEmptyString );
  if ( !SampleDir.IsOpened() )
  {
      wxMessageBox("moDirectorFrame constructor > Warning: samples directory not found!");
      exit(0);
  }
  ...
  //  fileMenu->Append(MODIRECTOR_NEWPREEFFECT, _T("Add P&reEffect\tAlt-W"), _T("Add a new pre-effect"));
    ...
    FrameManager.AddPane( m_pExplorerNotebook,      wxAuiPaneInfo().Name(wxT("explorer")).Caption(wxT("EXPLORER")).Left().Hide().CaptionVisible().Floatable().Movable().Dockable());
    FrameManager.AddPane( m_pFilesBook,         wxAuiPaneInfo().Name(wxT("filesbook")).Left().Hide().Caption(wxT("PARAMETERS")).CaptionVisible().Floatable().Movable().Dockable());
    FrameManager.AddPane( m_pInspectorNotebook, wxAuiPaneInfo().Name(wxT("inspector")).Right().Hide().Caption(wxT("INSPECTORS")).CaptionVisible().Floatable().Movable().Dockable());
    ...
    if (!m_pGUINotebook) FrameManager.AddPane( m_pPreviewWindow, wxAuiPaneInfo().Name(wxT("preview")).Left().Caption(wxT("OUTPUT")).CaptionVisible().Floatable().Movable().Dockable().MaximizeButton().MinimizeButton().Resizable(true));
    if (m_pGUINotebook) FrameManager.AddPane( m_pGUINotebook, wxAuiPaneInfo().Name(wxT("preview")).Left().Caption(wxT("OUTPUT")).CaptionVisible().Floatable().Movable().Dockable().MaximizeButton().MinimizeButton().Resizable(true));
        ...
        if (m_pGLCanvas) {
            ...
            */
            ...
            if (m_pGLContext==NULL) {
                cout << "Trying to create explicit GLContext..." << endl;
                m_pGLContext = new wxGLContext(m_pGLCanvas);
            }

            if (m_pGLContext==NULL) {
                    ErrorMessage(moText("Fatal Error: Couldn't create GL Context!"));
                    cout << "Fatal Error: Couldn't create GL Context!" << endl;
                    exit(1);
            }
...
void moDirectorFrame::OnExampleParticles(wxCommandEvent& event) {
	...
	wxString text = _("../../data/samples/particles_simple/particles_simple.mol");
	wxMessageBox(text);
    ProjectDescriptor.Set( moText("../../data/samples/particles_simple/"), moText("particles_simple.mol") );
    mStatus = OpenProject( ProjectDescriptor );
