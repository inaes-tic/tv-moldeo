...
//----------------------------------------------------------------------
   ...
   struct ProgramBase
   {
         ...
         virtual void compile(char const * * compilerArgs = 0) = 0;
         virtual void compile(std::vector<std::string> const& compilerArgs) = 0;
...
#  if defined(V3DLIB_GPGPU_ENABLE_CG)
   ...
   struct Cg_FragmentProgram : public ProgramBase, public Cg_ProgramBase
   {
         ...
         virtual void compile(char const * * compilerArgs = 0);
         virtual void compile(std::vector<std::string> const& compilerArgs);
         virtual void enable();
...
//----------------------------------------------------------------------
   ...
   enum GPUTextureSamplingPattern
   {
      ...
      GPU_SAMPLE_DIAG_NEIGBORS = 2, //!< Sample the diagonal neighbors (NE, SW, NW, SE)
      GPU_SAMPLE_2X2_BLOCK = 3, //!< Sample the 2x2 block (here, E, S, SE)
   };
